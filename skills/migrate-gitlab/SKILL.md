...
### Step 4: Update .gitlab-ci.yml and Related Scripts

Update CI configuration and auto-discover related scripts that may contain GOPRIVATE or git config.

#### 4.1 Update .gitlab-ci.yml
...
# This adds git config for new domain after old domain config
...
# Update other URLs (SRC_PATH, CWD, etc.)
...
#### 4.2 Auto-discover and Update Shell Scripts

Find and update scripts referenced in .gitlab-ci.yml and all shell scripts:
...
update_file() {
    ...
    # Update GOPRIVATE to include both domains
    if grep -q "GOPRIVATE=" "$file" 2>/dev/null; then
        sed -i '' 's|GOPRIVATE=.*|GOPRIVATE=git.meitu.com,gitlab.meitu.com|g' "$file"
    fi
    ...
    # Update other URLs (SRC_PATH, CWD, etc.)
    sed -i '' 's|gitlab.meitu.com/|git.meitu.com/|g' "$file"
}

# Update .gitlab-ci.yml
[ -f ".gitlab-ci.yml" ] && update_file ".gitlab-ci.yml"

# Find scripts referenced in .gitlab-ci.yml
if [ -f ".gitlab-ci.yml" ]; then
    grep -hE '^\s+-\s+\./' .gitlab-ci.yml 2>/dev/null | \
      sed -E 's/.*\.\//\./; s/\s.*//' | \
      grep -E '\.(sh|bash)$' | sort -u | while read -r script; do
        [ -f "$script" ] && update_file "$script"
    done
fi

# Find and update all .sh files
find . -name "*.sh" -type f ! -path "./vendor/*" ! -path "./.git/*" 2>/dev/null | while read -r file; do
    update_file "$file"
done
```
...
#### 4.4 Update Other CI Config Files

```bash
# Update other CI config files
for pattern in ".ci/*.yml" ".ci/*.yaml" "scripts/*.yml" "scripts/*.yaml" "deploy/*.yml" "deploy/*.yaml"; do
...
#### 4.5 Discover Files with GOPRIVATE/Git Config

To see which files will be affected before updating:
...
update_file() {
    ...
    # Update GOPRIVATE to include both domains
    if grep -q "GOPRIVATE=" "$file" 2>/dev/null; then
        sed -i '' "s|GOPRIVATE=.*|GOPRIVATE=$NEW_DOMAIN,$OLD_DOMAIN|g" "$file"
    fi

    # Update git config - keep old domain and add new domain
    sed -i '' "s|git config --global url.\"\\([^\"]*\\)@$OLD_DOMAIN/\"|git config --global url.\"\\1@$OLD_DOMAIN/\"\\n    git config --global url.\"\\1@$NEW_DOMAIN/\"|g" "$file"
...
# Update Makefile
[ -f "Makefile" ] && update_file "Makefile"
